...
export function TradesAiContactForm() {
  ...
  if (state.status === 'success') {
    return (
      <div className="flex flex-col items-center justify-center gap-4 rounded-2xl border border-zinc-800 bg-zinc-900 p-8 text-center">
        <div className="flex h-12 w-12 items-center justify-center rounded-full border border-zinc-700 text-xl text-white">
          ✓
        </div>
        <div>
          <p className="font-semibold text-white">Wiadomość wysłana.</p>
          <p className="mt-1 text-sm text-zinc-400">Odezwę się najdalej następnego dnia roboczego i zaproponuję prosty plan demo.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <form action={formAction} className="rounded-2xl border border-zinc-800 bg-zinc-900 p-8 shadow-2xl shadow-black/30">
      <div className="flex flex-col gap-5">
        <div>
          <label htmlFor="description" className="mb-1.5 block text-xs font-medium text-zinc-400">
            Napisz, kiedy najczęściej nie możesz odebrać telefonu
          </label>
          <textarea
            id="description"
            name="description"
      ...
      <p className="mt-4 text-center text-xs text-zinc-600">
        Bez spamu. Najpierw sprawdzimy, czy to ma sens dla Twojej firmy.
      </p>
    </form>
  );
}
